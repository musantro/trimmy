...
class CropWidget(QWidget):
    ...
    def init_crops(self):
        ...
        self.crops["bottom"] = CropRect(self.source_w * 0.2, self.source_h * 0.5, w, h)

    def set_crop_aspects(self, top_aspect, bottom_aspect):
    ...
    def paintEvent(self, event):
        ...
        if self.frame is None or self.source_w == 0:
            p.setPen(QColor("#666"))
            p.drawText(self.rect(), Qt.AlignCenter, "Open a video to begin")
            return
        ...
        p.drawImage(QRectF(ox, oy, dw, dh), self.frame, QRectF(0, 0, self.frame.width(), self.frame.height()))

        for key, color in [("top", QColor("#4ecdc4")), ("bottom", QColor("#ffe66d"))]:
            self._paint_crop(p, key, color)

    def _paint_crop(self, p: QPainter, key: str, color: QColor):
    ...
    def _crop_display_rect(self, key):
        c = self.crops[key]
        s = self._vid_scale
        return QRectF(self._vid_ox + c.x * s, self._vid_oy + c.y * s, c.w * s, c.h * s)

    def _handle_centers(self, key):
        r = self._crop_display_rect(key)
        return [
    ...
    def _widget_to_source(self, pos):
        return QPointF(
        ...
        for key in ("top", "bottom"):
            for hname, hcenter in self._handle_centers(key):
                if (pos - hcenter).manhattanLength() < hit:
                    cur = Qt.SizeFDiagCursor if hname in ("nw", "se") else Qt.SizeBDiagCursor
                    self.setCursor(cur)
                    return
            if self._crop_display_rect(key).contains(pos):
...
class PreviewWidget(QWidget):
    ...
    def set_crops(self, top: CropRect, bottom: CropRect):
    ...
    def paintEvent(self, event):
        ...
        if tc.w > 0 and tc.h > 0:
            p.drawImage(QRectF(0, 0, self.width(), top_h),
                        self.frame, QRectF(tc.x, tc.y, tc.w, tc.h))

        bc = self.crops["bottom"]
        if bc.w > 0 and bc.h > 0:
            p.drawImage(QRectF(0, top_h, self.width(), bot_h),
                        self.frame, QRectF(bc.x, bc.y, bc.w, bc.h))
...
class TimelineWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        p.drawRoundedRect(QRectF(sx, bar.top(), ex - sx, bar.height()), 6, 6)

        hw = 12
        for t in (self.trim_start, self.trim_end):
            hx = self._t2x(t)
            p.setBrush(QColor("#e94560"))
            p.drawRoundedRect(QRectF(hx - hw / 2, bar.top() - 4, hw, bar.height() + 8), 3, 3)

        phx = self._t2x(self.position)
        p.setPen(QPen(Qt.white, 2))
        p.drawLine(QPointF(phx, bar.top()), QPointF(phx, bar.bottom()))
        ...
        p.setFont(font)
        y = bar.bottom() + 4
        p.drawText(QRectF(bar.left(), y, 120, 18), Qt.AlignLeft, self._fmt(self.trim_start))
        dur_text = f"Duration: {self._fmt(self.trim_end - self.trim_start)}"
        p.drawText(QRectF(bar.left(), y, bar.width(), 18), Qt.AlignCenter, dur_text)
        p.drawText(QRectF(bar.right() - 120, y, 120, 18), Qt.AlignRight, self._fmt(self.trim_end))
