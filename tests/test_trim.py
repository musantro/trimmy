...
def test_planner_splits_into_parts():
    segments = SegmentPlanner().plan(TrimRange(0.0, 25.0), 10.0)
    assert len(segments) == 3
    assert segments[0] == Segment(1, 3, 0.0, 10.0)
    assert segments[1] == Segment(2, 3, 10.0, 20.0)
    assert segments[2] == Segment(3, 3, 20.0, 25.0)
