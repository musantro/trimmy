...
def test_probe_video_use_case():
    metadata = ProbeVideoUseCase(FakeProber()).execute(
...
def test_render_cpu_success():
    ...
    outcome = RenderVideoUseCase(_presets(), backend).execute(make_spec())
...
def test_segments_single_part():
    ...
    result = RenderSegmentsUseCase(_presets(), backend).execute(
